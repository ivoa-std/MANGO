...
that have to interpret them.
  \subsection{EpochPosition}
    ...
    \textbf{type: \hyperref[sect:Epoch]{mango:Epoch}} \newline
    \textbf{multiplicity: 1} \newline
  ...
  \subsection{Epoch}
    \label{sect:Epoch}
    Datatype for timestamps and their representation. Timestamps are modeled by a single value and a representation mode. There is no time scale associated with \texttt{mango:Epoch}. The scale is either imposed by the representation mode or defined by the time coordinate system relevant for the host object, usually an instance of \texttt{mango:EpcohPosition}.

    \subsubsection{Epoch.representation}
    \textbf{vodml-id: Epoch.representation} \newline
    \textbf{type: \hyperref[sect:ivoa]{ivoa:string}} \newline
    \textbf{multiplicity: 1} \newline
    Define the representation mode. The representation mode must comply with the \texttt{time-format} IVOA vocabulary.

    \subsubsection{Epoch.value}
    \textbf{vodml-id: Epoch.value} \newline
    \textbf{type: \hyperref[sect:ivoa]{ivoa:string}} \newline
    \textbf{multiplicity: 1} \newline
    The timestamp value is always given as a string. The way to process this string is given by the representation mode. It can represent a date, which must be parsed, or a real value, which must be cast.
...
This section brings together all the classes needed to operate a hook on 
objects imported from the \texttt{Measurements} data model \citep{2022ivoa.specQ1004R}. 
The Measurements model contains 6 classes that model simple physical quantities 
 (Position, Time, Velocity, Polarisation, ProperMotion, GenericMeasure) in a very versatile way,
supporting many coordinate systems and a wide variety of errors.

It is recommended to use them for quantities that do not require the use of native MANGO classes.
