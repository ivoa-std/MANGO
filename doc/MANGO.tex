...
\TODO{a integrer mieux dans le texte}
Mango is designed to enhance the tabular data provided by VO services running any protocol.
It can provide a global view that describes a table row as a single entity, or a sparse set of unrelated properties.
The natural way to connect data and MANGO is MIVOT which is a generic XML syntax mapping data on any model.
...
\begin{itemize}
    ...
    \item \emph{MANGO as unrelated views}: In this simpler approach the data mapping is made of a
...
\begin{itemize}
  ...
  \item \texttt{popertyDock}: place holder for all the \texttt{MANGOObject} properties.
        ...
        Properties can be linked together to represent logical parameter sets for which
        the model does not provide semantics.
\end{itemize}
