...
From the above use-case list, we have identified 4 domains for which
the model should provide real added value: 1) set of supported quantities 2) data description enhancement,
...
\begin{itemize}
    ...
    \item Metadata enhancement:
        \begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt]
          \item MANGO must support a convenient way to identify model instance.
          \item MANGO must be able to attach relevant coordinate (or calibration) 
                system to any quantities. 
          ...
          \item MANGO must be able to specify the set of allowed values for quantities which purpose is to flag data 
                (e.g. detection flag). It also be able to provide a description for each of these values. 
                This model feature will provide a straightforward way of telling the user 
                what a current flag value means.
          \item MANGO must be able to provide information about the origin of the modeled data set.         
          \item In the case of datasets issued by services not operating DataLinks services but providing 
    ...
    \item Multi-columns quantities
       \begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt]
          \item MANGO must be able to provide an accurate description of quantities which parameters are spread 
                out on multiple columns (e.g. position, errors).
          ...
          \item MANGO must provide an accurate description of the epoch propagation. 
                ...
                This feature is required to compare positions given by high astrometry accuracy surveys such as GAIA.
       \end{itemize} 
         
    \item Connected quantities : 
       \begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt]
           \item MANGO must be able to setup links between different parameters of the same table. 
                 This can be relevant for instance for attaching a detection likelihoods with source positions
...
Identifiers should be unique within a collection e.g. a data table.
...
\begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt]
  ...
  \item \texttt{popertyDock}: place holder for all the \texttt{MANGOObject} properties.
        ...
        Properties can be linked together to compound parameters.
\end{itemize}
...
Data rows MANGO can be mapped o,n MANGO in two ways: 

\begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt]
    \item Each row is considered as a single object where the 
          properties are grouped in a dock.
    \item Each row is considered as a scattered collection of independent quantities.
\end{itemize}

MIVOT annotations support both MANGO uses:
...
\begin{itemize}
    \item \emph{MANGO as a global view}: This configuration is necessary to take advantage of all 
          ...
          and to add a few more class levels to the annotations.
    \item \emph{MANGO as a sparse
     parameter view}: In this simpler approach the data mapping is made of a
          flat set on independant objects. Clients can iterate over these objects and 
          process the entities of interest. 
          It is to be noted that such a client could also easily process data mapped 
          on the full MANGO scheme as described above. 
