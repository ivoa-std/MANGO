...
Sky objects are represented by instances of the class \texttt{MangoObject} which has only one mandatory attribute,
the source \texttt{identifier}.
It is recommended the identifiers to be unique within a source collection e.g. a catalog.

The \texttt{MangoObject} is a dock for all source properties and for all data associated to that object.
The pattern of either parameters or associated data attached to a source is not specified by the model.
It depends of the data set on which the model is applied.

The \texttt{MangoObject} has one connector for the parameters, the \texttt{parameters} relation,
and another for the associatedData, the  \texttt{associatedData}  relations.
...
Each associated data or data pointer is hooked to the \texttt{MangoObject} by a wrapping class (\texttt{AssociatedData})
...
Mango provides 5 description levels for each property:
...
\begin{figure}
     \includegraphics[width=1.0\textwidth]{../model/mangoProperty.png}
...
\begin{figure}
  \includegraphics[width=1.0\textwidth]{../model/MangoCorrelation.png}
  \caption{Mango correlated errors.}
